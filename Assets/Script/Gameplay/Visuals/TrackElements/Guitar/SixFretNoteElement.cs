    ...
    public sealed class SixFretNoteElement : NoteElement<GuitarNote, SixFretPlayer>
    {
        ...
        protected override void InitializeElement()
        {
            ...
            if (NoteRef.Fret != (int) SixFretGuitarFret.Open)
            {
                ...
                if (NoteRef.Fret > 3)
                {
                    SustainLine[] sus = transform.GetComponentsInChildren<SustainLine>();
                    foreach (SustainLine line in sus)
                    { 
                        line.gameObject.transform.localRotation = Quaternion.Euler(180f, 0f, 0f);
                    }
                ...
                else
                {
                    ...
                    foreach (SustainLine line in sus)
                    { 
                        line.gameObject.transform.localRotation = Quaternion.Euler(0f, 0f, 0f);
                    }
                    transform.localRotation = Quaternion.Euler(0f, 0f, 0f);
                }

                // Set the position
                transform.localPosition = new Vector3(GetElementX(NoteRef.Fret, 6), 0.1f, 0f) * LeftyFlipMultiplier;
